//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "ArtcEngineDelegate.h"
#import "RTCEAGLVideoViewDelegate.h"

@class ARTCManagerProtocol, NSString, RTCEAGLVideoView, TBAccsManager;

@interface ARTCEngineKit : NSObject <RTCEAGLVideoViewDelegate, ArtcEngineDelegate>
{
    RTCEAGLVideoView *_videoView1;
    RTCEAGLVideoView *_videoView2;
    _Bool _isView1LocalView;
    _Bool _isView2LocalView;
    struct ArtcManager *_manager;
    ARTCManagerProtocol *_managerProtocol;
    basic_string_a490aa4c _peerId;
    basic_string_a490aa4c _deviceID;
    basic_string_a490aa4c _channelID;
    basic_string_a490aa4c _serviceName;
    struct recursive_mutex _mtxManager;
    _Bool _isPreviewON;
    _Bool _isSessionON;
    _Bool _hadRecvFirstFrame;
    _Bool _isSounderOpened;
    _Bool _hadSetSpeakerphone;
    _Bool _isBindAppSuccess;
    _Bool _isBindServiceSuccess;
    _Bool _isMirrored;
    _Bool _isAnchor;
    _Bool _isBackCamera;
    _Bool _ready2RecvSignalingData;
    int _channelProfile;
    int _orientation;
    NSString *_currentAudioSessionCategory;
    _Bool _hadInitManager;
    basic_string_a490aa4c _callID;
    id <ArtcEngineDelegate> _callback;
    TBAccsManager *_accsManager;
}

+ (id)staticMethodCreateChannel:(id)arg1 serviceName:(id)arg2 bizId:(id)arg3;
+ (id)getInstance;
@property(retain, nonatomic) TBAccsManager *accsManager; // @synthesize accsManager=_accsManager;
@property(nonatomic) __weak id <ArtcEngineDelegate> callback; // @synthesize callback=_callback;
- (id).cxx_construct;
- (void).cxx_destruct;
- (void)onCpuOverload;
- (void)getChannelUserList:(id)arg1;
- (void)setBroadcastRole:(int)arg1;
- (void)setConsoleLogSwitch:(_Bool)arg1;
- (void)onUserInChannel:(id)arg1;
- (void)onChannelUserList:(id)arg1;
- (void)onAudioFrameProcess:(_Bool)arg1 audio10ms:(short *)arg2 length:(unsigned long long)arg3 samplingFreq:(int)arg4 isStereo:(_Bool)arg5;
- (void)onUseBackCamera:(_Bool)arg1;
- (void)onVideoFrameCaptured:(void *)arg1 rotation:(int)arg2;
- (void)onCancelCall:(id)arg1 remoteUserID:(id)arg2 errCode:(int)arg3 errMsg:(id)arg4 exten:(id)arg5;
- (void)onCanceledCallEX:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (void)onCanceledCall:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (void)onKicked:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (void)onKick:(id)arg1 remoteUserID:(id)arg2 errCode:(int)arg3 errMsg:(id)arg4 exten:(id)arg5;
- (void)onAnswered:(id)arg1 callID:(id)arg2 userID:(id)arg3 result:(int)arg4 role:(int)arg5 exten:(id)arg6;
- (void)onAnswer:(id)arg1 callID:(id)arg2 result:(int)arg3 resultMsg:(id)arg4 exten:(id)arg5;
- (void)onCallTimeout:(id)arg1 userID:(id)arg2;
- (void)onInvited:(id)arg1 callID:(id)arg2 userID:(id)arg3 result:(_Bool)arg4 exten:(id)arg5;
- (void)onInvite:(id)arg1 callID:(id)arg2 userID:(id)arg3 result:(int)arg4 resultMsg:(id)arg5 exten:(id)arg6;
- (void)onCalled:(id)arg1 callID:(id)arg2 userID:(id)arg3 exten:(id)arg4;
- (void)onCalled:(id)arg1 callID:(id)arg2 userID:(id)arg3 result:(_Bool)arg4 role:(int)arg5 exten:(id)arg6;
- (void)onCall:(id)arg1 callID:(id)arg2 userID:(id)arg3 result:(int)arg4 resultMsg:(id)arg5 exten:(id)arg6;
- (void)onLastmileQuality:(int)arg1;
- (void)onChannelIdUpdated:(id)arg1;
- (void)onFirstRemoteVideoFrame:(int)arg1 height:(int)arg2 time:(int)arg3;
- (void)onConnectionLost;
- (void)onConnectionInterrupted;
- (void)onArtcStats:(id)arg1;
- (void)onArtcMsg:(id)arg1 level:(int)arg2;
- (void)onError:(int)arg1 code:(int)arg2 errorMsg:(id)arg3;
- (void)onLeaveChannel:(id)arg1;
- (void)onChannelClosed:(id)arg1 reason:(id)arg2 reasonId:(int)arg3 exten:(id)arg4;
- (void)onUserLeftChannel:(id)arg1 leftReason:(int)arg2 exten:(id)arg3;
- (void)onUserJoinedChannel:(id)arg1;
- (void)onJoinChannelSuccess:(id)arg1 time:(int)arg2;
- (void)onRejoinChannelSuccess:(id)arg1;
- (void)onCreateChannelSuccess:(id)arg1;
- (void)onInitializeSuccess;
- (void)videoView:(id)arg1 didChangeVideoSize:(struct CGSize)arg2;
- (void)bindApp;
- (void)bindAppWithAppleToken:(id)arg1 resultsBlock:(CDUnknownBlockType)arg2;
- (void)bindService;
- (void)unRegisterUser:(id)arg1;
- (void)registerUser:(id)arg1;
- (_Bool)queryUserInChannel:(_Bool)arg1;
- (_Bool)isMediaGuaranteed:(_Bool)arg1;
- (_Bool)setVideoLayout:(id)arg1;
- (void)setupAccs;
- (void)onWillEnterForeground:(id)arg1;
- (void)onDidEnterBackground:(id)arg1;
- (void)onWillResignActive:(id)arg1;
- (void)onDidBecomeActive:(id)arg1;
- (_Bool)switchAudioCategaryWithSpeaker:(_Bool)arg1;
- (_Bool)isHeadPhoneEnable;
- (void)audioRouteChangeListenerCallback:(id)arg1;
- (void)onRecvSignalingMessage:(id)arg1;
- (_Bool)setBroadcastConfig:(id)arg1;
- (_Bool)setCaptureVideoOrientation:(int)arg1;
- (_Bool)enableFaceBeauty:(_Bool)arg1;
- (_Bool)muteRemoteVideoStream:(_Bool)arg1;
- (_Bool)muteRemoteAudioStream:(_Bool)arg1;
- (_Bool)muteLocalVideoStream:(_Bool)arg1;
- (_Bool)muteLocalAudioStream:(_Bool)arg1;
- (_Bool)setTorch:(_Bool)arg1;
- (void)switchCamera;
- (void)swapScreen;
- (void)stopPreview;
- (_Bool)startPreview;
- (_Bool)setRemoteView:(id)arg1;
- (_Bool)setLocalView:(id)arg1;
- (void)setLocalViewMirrorOrNot;
- (void)setLocalViewMirror:(_Bool)arg1;
- (void)setFrontCameraMirror:(_Bool)arg1;
- (_Bool)cancelCallPstn:(id)arg1 phoneNumber:(id)arg2 exten:(id)arg3;
- (_Bool)cancelCallDevices:(id)arg1 devices:(id)arg2 exten:(id)arg3;
- (_Bool)cancelCall:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (_Bool)cancelCall:(id)arg1 remoteUserID:(id)arg2;
- (_Bool)kick:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (_Bool)kick:(id)arg1 remoteUserID:(id)arg2;
- (_Bool)answer:(id)arg1 callId:(id)arg2 remoteUserID:(id)arg3 answer:(int)arg4 exten:(id)arg5;
- (_Bool)answer:(id)arg1 callId:(id)arg2 remoteUserID:(id)arg3 answer:(int)arg4;
- (_Bool)invite:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (_Bool)callPstn:(id)arg1 phoneNumber:(id)arg2 exten:(id)arg3;
- (_Bool)callDevices:(id)arg1 devices:(id)arg2 exten:(id)arg3;
- (_Bool)call:(id)arg1 remoteUserID:(id)arg2 exten:(id)arg3;
- (_Bool)isSpeakerphoneEnabled;
- (_Bool)setEnableSpeakerphone:(_Bool)arg1;
- (_Bool)leaveChannel:(id)arg1;
- (_Bool)leaveChannel;
- (_Bool)joinChannel:(id)arg1;
- (_Bool)createChannel:(id)arg1;
- (id)createNewChannel:(id)arg1;
- (void)setAudioOutputVolume:(double)arg1;
- (_Bool)setVideoResolution:(int)arg1 isSwap:(_Bool)arg2;
- (_Bool)setVideoProfile:(int)arg1 isSwap:(_Bool)arg2;
- (_Bool)setChannelProfile:(int)arg1 isVideoCall:(_Bool)arg2;
- (_Bool)sendVideoFrame:(void *)arg1 rotation:(int)arg2;
- (void)setAVCaptureCallbackSwitch:(int)arg1 enabled:(_Bool)arg2;
- (void)setCallTimeout:(int)arg1;
- (void)changeUserId:(id)arg1;
- (id)getVersion;
- (void)destroy;
- (void)unInitialize;
- (void)sharedEngineWithOption:(id)arg1 delegate:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

