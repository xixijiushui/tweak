//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

@interface UIView (SKAdditions)
- (void)hideOneFourthRoundView;
- (void)showOneFourthRoundViewBlue;
- (void)showOneFourthRoundViewWithColor:(id)arg1;
- (void)setOneFourthRoundView:(id)arg1;
- (id)oneFourthRoundView;
- (void)stopRequestLoadingAnimation;
- (void)startRequestLoadingAnimation;
- (void)setRequestLoadingView:(id)arg1;
- (id)requestLoadingView;
- (void)removeAllSubviews;
- (id)ancestorOrSelfWithClass:(Class)arg1;
- (id)descendantOrSelfWithClass:(Class)arg1;
@property(nonatomic) struct CGSize size;
@property(nonatomic) struct CGPoint origin;
@property(readonly, nonatomic) double screenViewY;
@property(readonly, nonatomic) double screenViewX;
@property(nonatomic) double height;
@property(nonatomic) double width;
@property(nonatomic) double centerY;
@property(nonatomic) double centerX;
@property(nonatomic) double bottom;
@property(nonatomic) double right;
@property(nonatomic) double top;
@property(nonatomic) double left;
@end

