//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class HYStackView, NSArray, NSMutableArray;

@interface HYStackViewDistributionStrategy : NSObject
{
    HYStackView *_stackView;
    NSMutableArray *_constraints;
}

+ (id)strategyWithStackView:(id)arg1;
@property(retain, nonatomic) NSMutableArray *constraints; // @synthesize constraints=_constraints;
@property(nonatomic) __weak HYStackView *stackView; // @synthesize stackView=_stackView;
- (void).cxx_destruct;
- (id)symbolicSpacingRelation;
- (void)removeAddedConstraints;
@property(readonly, nonatomic) NSArray *addedConstraints;
- (double)lastMargin;
- (double)firstMargin;
- (long long)equalityAxis;
- (id)currentAxisString;
- (void)alignMiddleView:(id)arg1 afterView:(id)arg2;
- (void)alignFirstView:(id)arg1;
- (void)alignLastView:(id)arg1;
- (void)alignView:(id)arg1 afterView:(id)arg2;
- (id)initStackView:(id)arg1;

@end

