//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

@class UIColor;

@interface AliRecordProgressView : UIView
{
    UIColor *_progressShineColor;
    UIView *_progressView;
    double _progress;
}

@property(nonatomic) double progress; // @synthesize progress=_progress;
@property(retain, nonatomic) UIView *progressView; // @synthesize progressView=_progressView;
@property(nonatomic) UIColor *progressShineColor; // @synthesize progressShineColor=_progressShineColor;
- (void).cxx_destruct;
@property(nonatomic) UIColor *progressBackgroundColor;
- (void)setProgress:(double)arg1 animated:(_Bool)arg2;
- (void)layoutSubviews;
- (id)initWithFrame:(struct CGRect)arg1;

@end

