//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSMethodSignature;

@interface ALHBlockInvocation : NSObject
{
    NSMethodSignature *_methodSignature;
    id _block;
    NSMethodSignature *_blockSignature;
}

+ (id)methodSignatureForBlock:(id)arg1;
+ (id)methodSignatureForBlockSignature:(id)arg1;
+ (id)typeSignatureForBlock:(id)arg1;
+ (_Bool)isSignature:(id)arg1 compatibleWithSignature:(id)arg2;
@property(readonly, nonatomic) NSMethodSignature *blockSignature; // @synthesize blockSignature=_blockSignature;
@property(readonly, copy, nonatomic) id block; // @synthesize block=_block;
@property(readonly, nonatomic) NSMethodSignature *methodSignature; // @synthesize methodSignature=_methodSignature;
- (void).cxx_destruct;
- (_Bool)invokeWithInvocation:(id)arg1 returnValue:(out id *)arg2;
- (void)invokeWithInvocation:(id)arg1;
- (_Bool)invokeWithInvocation:(id)arg1 returnValue:(out id *)arg2 setOnInvocation:(_Bool)arg3;
- (id)initWithBlock:(id)arg1 methodSignature:(id)arg2;
- (id)initWithBlock:(id)arg1;
- (id)initWithBlock:(id)arg1 methodSignature:(id)arg2 blockSignature:(id)arg3;

@end

